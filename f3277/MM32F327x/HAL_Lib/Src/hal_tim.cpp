...
namespace TIM
{
    const uint32_t timBs[] = {TIM1_BASE, TIM2_BASE, TIM3_BASE, TIM4_BASE, TIM5_BASE, TIM6_BASE, TIM7_BASE, TIM8_BASE};
#define timIC_Init_Func(tim, ch)                                                  \
    TI##ch##_Configure(tim, TIM_ICPolarity_Falling, TIM_ICSelection_DirectTI, 0); \
    TIM_SetIC##ch##Prescaler(tim, TIM_ICPSC_DIV1);

#define timIC_case(ch)                                          \
    case (uint8_t)##ch##:                                       \
        timIC_Init_Func((TIM_TypeDef *)timBs[tim - 1], ##ch##); \
        break
