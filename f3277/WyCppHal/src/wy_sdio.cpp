...
SD_Error SD_PowerON(void)
{
    ...
    if (errorstatus == SD_OK)
    { // SD2.0/SD 1.1,Otherwise it's an MMC card
...
SD_Error SD_GetCardInfo(SD_CardInfo *cardinfo)
{
    ...
    {                                                     // Standard 1.1/2.0 card /MMC card
...
void CMD_Send(unsigned int cmd, unsigned int arg)
{
    ...
    while (1)
    {
        if (SDIO->CLR_MMC_INT & 0x1)
        {                            // judge which interrupt generation
...
#define USE_BUFFER_FULL // USE_BUFFER_EMPTY//
        ...
        { // FIFO is full , available for 512 Bytes
            for (j = 0; j < 128; j++)
            {
                *(vu32 *)tempbuff++ = SDIO->DATA_BUF0;
            }
