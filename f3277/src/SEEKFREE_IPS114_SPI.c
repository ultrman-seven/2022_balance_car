...
#define DC_P_S GPIO_PinSource7
#define CS_P_S GPIO_PinSource8
#define RST_P_S GPIO_PinSource9

#define DC_PIN (0x01 << DC_P_S)
#define CS_PIN (0x01 << CS_P_S)
#define RST_PIN (0x01 << RST_P_S)

#define DC GPIOB, DC_PIN
#define CS GPIOB, CS_PIN
#define RST GPIOB, RST_PIN

#define IPS114_CS(n) n ? GPIO_SetBits(CS) : GPIO_ResetBits(CS)
#define IPS114_DC(n) n ? GPIO_SetBits(DC) : GPIO_ResetBits(DC)
#define IPS114_REST(n) n ? GPIO_SetBits(RST) : GPIO_ResetBits(RST)
...
{
	while (l--)
		SPI_SendData(IPS_SPI, *dat++);
}
static void ips114_writeIndex(uint8_t dat)
{
	IPS114_CS(0);
	IPS114_DC(0);
	spi_mosi(0, &dat, NULL, 1);
	IPS114_CS(1);
}
...
void ips114_init(void)
{
	...
	g.GPIO_Pin = DC_PIN | CS_PIN | RST_PIN | GPIO_Pin_6;
	...
	SPI_Init(IPS_SPI, &init_spi);
	SPI_Cmd(IPS_SPI, ENABLE);
	SPI_BiDirectionalLineConfig(IPS_SPI, SPI_Direction_Tx);
	SPI_BiDirectionalLineConfig(IPS_SPI, SPI_Direction_Rx);
